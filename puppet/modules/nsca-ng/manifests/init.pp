...
class nsca-ng-server {
  ...
  file { '/etc/nsca-ng.cfg':
    source => 'puppet:////opt/config/puppet/files/etc/nsca-ng.cfg',
...
class nsca-ng-client {
  ...
  file { '/etc/icinga2/conf.d/passive.conf':
    source => 'puppet:////opt/config/puppet/files/etc/icinga2/conf.d/passive.conf',
    require => Package['nsca-ng-client'],
    notify => Service['icinga2']
  }

  file { '/etc/send_nsca.cfg':
    source => 'puppet:////opt/config/puppet/files/etc/send_nsca.cfg',
    owner => 'root',
    group => 'root',
    mode => '0600',
    require => Package['nsca-ng-client'],
  ...
  file { '/home/vagrant/passive_result':
    source => 'puppet:////opt/config/puppet/files/home/vagrant/passive_result',
    require => Package['nsca-ng-client'],
  }
}